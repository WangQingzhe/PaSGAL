...
#include <time.h>
#if defined(PASGAL_ENABLE_AVX512) || defined(PASGAL_ENABLE_AVX2) || defined(PASGAL_ENABLE_NEON128)
...
#pragma omp for schedule(dynamic) nowait
                         ...
                         std::vector<BestScoreInfo> &outputBestScoreVector)
    {
        ...
        //
        {
            ...
            for (size_t readno = 0; readno < readSet.size(); readno++)
            {
                ...
                // std::cout << readno << ":" << outputBestScoreVector[readno].score << " in [" << outputBestScoreVector[readno].qryRowEnd << "," << outputBestScoreVector[readno].refColumnEnd << "]\n";
                    ...
                    std::vector<BestScoreInfo> &outputBestScoreVector)
    {
        ...
        {
        case LOCAL:
            alignToDAGLocal(reads, graph, parameters, outputBestScoreVector);
            break;
        default:
            std::cerr << "ERROR, psgl::alignToDAG, Invalid alignment mode";
            exit(1);
        }
    }
