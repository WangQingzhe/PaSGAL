...
#include <immintrin.h>
#include <x86intrin.h>
#elif defined(PASGAL_ENABLE_NEON128)
#include <arm_neon.h>
#endif
...
namespace psgl
{
    ...
    struct SimdInst<int32_t>
    {
...
#elif defined(PASGAL_ENABLE_NEON128)
        // 32x4向量寄存器
        typedef int32x4_t regType;
        // 列位置寄存器
        typedef int32x4_t col_regType;
        static constexpr int numSeqs = SIMD_REG_SIZE / (8 * sizeof(type));
        // 2个32x4向量寄存器相加
        static inline regType add(const regType &a, const regType &b) { return vaddq_s32(a, b); }
        // 将32x4寄存器每个向量通道置为a
        static inline regType set1(int32_t a) { return vdupq_n_s32(a); }
        // 将32x4寄存器每个向量通道置为a
        static inline col_regType set1_32(int32_t a) { return vdupq_n_s32(a); }
        // 返回两个32x4寄存器相应通道的较大值
        static inline regType max(const regType &a, const regType &b) { return vmaxq_s32(a, b); }
        // 返回全0的32x4寄存器
        static inline regType zero() { return vdupq_n_s32(0); }
        // 返回全0的32x4寄存器
        static inline int32x4_t zero_32() { return vdupq_n_s32(0); }
        // 将向量寄存器的值存入内存中
        static inline void store(regType *mem_addr, const regType &a) { vst1q_s32((int32_t *)mem_addr, a); }
        // 将向量寄存器的值存入内存中
        static inline void store_32(col_regType *mem_addr, const col_regType &a) { vst1q_s32((int32_t *)mem_addr, a); }
        // 从内存中取出向量寄存器的值
        static inline regType load(const regType *mem_addr) { return vld1q_s32((int32_t *)mem_addr); }
        // 从内存中取出向量寄存器的值
        static inline col_regType load_32(const col_regType *mem_addr) { return vld1q_s32((int32_t *)mem_addr); }
        // 比较两个32x4寄存器各向量通道是否相等
        static inline uint32x4_t cmpeq(const regType &a, const regType &b) { return vceqq_s32(a, b); }
        // 比较两个32x4寄存器个向量通道是否相等
        static inline uint32x4_t cmpeq_32(const col_regType &a, const col_regType &b) { return vceqq_s32(a, b); }
        // 根据k的0/1返回a/b
        static inline regType blend(uint32x4_t k, const regType &a, const regType &b) { return vbslq_s32(k, b, a); }
        // 根据k的1/0判断是否用b覆盖a
        static inline regType mask_set1(const regType &a, uint32x4_t uk, int32_t b)
        {
            // 向量c每个通道都置为b
            int32x4_t c = vdupq_n_s32(b);
            return vbslq_s32(uk, c, a);
        }
        // 根据k的1/0判断是否用b覆盖a
        static inline col_regType mask_set1_32(const col_regType &a, uint32x4_t uk, int32_t b)
        {
            // 向量c每个通道都置为b
            int32x4_t c = vdupq_n_s32(b);
            return vbslq_s32(uk, c, a);
        ...
        // 返回(k3k2k1k0)
        static inline uint32x4_t combine_mask(uint32x4_t k0, uint32x4_t k1, uint32x4_t k2, uint32x4_t k3) { return k0; }
        // 根据k的1/0判断是否用val覆盖c0123
        static inline void update_cols(col_regType &c0, col_regType &c1, col_regType &c2, col_regType &c3, int32_t val, uint32x4_t k)
        {
    ...
    struct SimdInst<int16_t>
    {
        ...
        static inline __mxxxi mask_set1_32(const __mxxxi &a, mmask_T k, int32_t b) { return _mm512_mask_set1_epi32(a, k, b); }
        static inline mmask_t combine_mask(mmask_T k0, mmask_T k1, mmask_T k2, mmask_T k3) { return ((mmask_t)k0) | ((mmask_t)k1) << 16; }
...
#elif defined(PASGAL_ENABLE_NEON128)
        // 16x8向量寄存器
        typedef int16x8_t regType;
        // 列位置寄存器
        typedef int32x4_t col_regType;
        static constexpr int numSeqs = SIMD_REG_SIZE / (8 * sizeof(type));
        // 2个16x8向量寄存器相加
        static inline regType add(const regType &a, const regType &b) { return vaddq_s16(a, b); }
        // 将16x8寄存器每个向量通道置为a
        static inline regType set1(int16_t a) { return vdupq_n_s16(a); }
        // 将32x4寄存器每个向量通道置为a
        static inline col_regType set1_32(int32_t a) { return vdupq_n_s32(a); }
        // 返回两个16x8寄存器相应通道的较大值
        static inline regType max(const regType &a, const regType &b) { return vmaxq_s16(a, b); }
        // 返回全0的16x8寄存器
        static inline regType zero() { return vdupq_n_s16(0); }
        // 返回全0的32x4寄存器
        static inline int32x4_t zero_32() { return vdupq_n_s32(0); }
        // 将向量寄存器的值存入内存中
        static inline void store(regType *mem_addr, const regType &a) { vst1q_s16((int16_t *)mem_addr, a); }
        // 将向量寄存器的值存入内存中
        static inline void store_32(col_regType *mem_addr, const col_regType &a) { vst1q_s32((int32_t *)mem_addr, a); }
        // 从内存中取出向量寄存器的值
        static inline regType load(const regType *mem_addr) { return vld1q_s16((int16_t *)mem_addr); }
        // 从内存中取出向量寄存器的值
        static inline col_regType load_32(const col_regType *mem_addr) { return vld1q_s32((int32_t *)mem_addr); }
        // 比较两个16x8寄存器各向量通道是否相等
        static inline uint16x8_t cmpeq(const regType &a, const regType &b) { return vceqq_s16(a, b); }
        // 比较两个32x4寄存器个向量通道是否相等
        static inline uint32x4_t cmpeq_32(const col_regType &a, const col_regType &b) { return vceqq_s32(a, b); }
        // 根据k的0/1返回a/b
        static inline regType blend(uint16x8_t k, const regType &a, const regType &b) { return vbslq_s16(k, b, a); }
        // 根据k的1/0判断是否用b覆盖a
        static inline regType mask_set1(const regType &a, uint16x8_t uk, int16_t b)
        {
            // 向量c每个通道都置为b
            int16x8_t c = vdupq_n_s16(b);
            return vbslq_s16(uk, c, a);
        }
        // 根据k的1/0判断是否用b覆盖a
        static inline col_regType mask_set1_32(const col_regType &a, uint32x4_t uk, int32_t b)
        {
        ...
        // 返回(k1k0)
        static inline uint16x8_t combine_mask(uint32x4_t k0, uint32x4_t k1, uint32x4_t k2, uint32x4_t k3)
        {
            uint16x8_t k = vdupq_n_u16(0);
            if (vgetq_lane_u32(k0, 0) == UINT32_MAX)
                k = vsetq_lane_u16(UINT16_MAX, k, 0);
            if (vgetq_lane_u32(k0, 1) == UINT32_MAX)
                k = vsetq_lane_u16(UINT16_MAX, k, 1);
            if (vgetq_lane_u32(k0, 2) == UINT32_MAX)
                k = vsetq_lane_u16(UINT16_MAX, k, 2);
            if (vgetq_lane_u32(k0, 3) == UINT32_MAX)
                k = vsetq_lane_u16(UINT16_MAX, k, 3);
            if (vgetq_lane_u32(k1, 0) == UINT32_MAX)
                k = vsetq_lane_u16(UINT16_MAX, k, 4);
            if (vgetq_lane_u32(k1, 1) == UINT32_MAX)
                k = vsetq_lane_u16(UINT16_MAX, k, 5);
            if (vgetq_lane_u32(k1, 2) == UINT32_MAX)
                k = vsetq_lane_u16(UINT16_MAX, k, 6);
            if (vgetq_lane_u32(k1, 3) == UINT32_MAX)
                k = vsetq_lane_u16(UINT16_MAX, k, 7);
            return k;
        }
        // 根据k的1/0判断是否用val覆盖c01
        static inline void update_cols(col_regType &c0, col_regType &c1, col_regType &c2, col_regType &c3, int32_t val, uint16x8_t k)
        {
            if (vgetq_lane_u16(k, 0) == UINT16_MAX)
                c0 = vsetq_lane_s32(val, c0, 0);
            if (vgetq_lane_u16(k, 1) == UINT16_MAX)
                c0 = vsetq_lane_s32(val, c0, 1);
            if (vgetq_lane_u16(k, 2) == UINT16_MAX)
                c0 = vsetq_lane_s32(val, c0, 2);
            if (vgetq_lane_u16(k, 3) == UINT16_MAX)
                c0 = vsetq_lane_s32(val, c0, 3);
            if (vgetq_lane_u16(k, 4) == UINT16_MAX)
                c1 = vsetq_lane_s32(val, c1, 0);
            if (vgetq_lane_u16(k, 5) == UINT16_MAX)
                c1 = vsetq_lane_s32(val, c1, 1);
            if (vgetq_lane_u16(k, 6) == UINT16_MAX)
                c1 = vsetq_lane_s32(val, c1, 2);
            if (vgetq_lane_u16(k, 7) == UINT16_MAX)
                c1 = vsetq_lane_s32(val, c1, 3);
        }
    ...
    struct SimdInst<int8_t>
    {
        ...
        // 16位整型for 32-bit integer operations
        typedef SimdInst<int32_t>::mmask_t mmask_T;
        // 2个512位向量寄存器相加
...
#elif defined(PASGAL_ENABLE_NEON128)
        // 8x16向量寄存器
        typedef int8x16_t regType;
        // 列位置寄存器
        typedef int32x4_t col_regType;
        static constexpr int numSeqs = SIMD_REG_SIZE / (8 * sizeof(type));
        // 2个8x16向量寄存器相加1
        static inline regType add(const regType &a, const regType &b) { return vaddq_s8(a, b); }
        // 将8x16寄存器每个向量通道置为a1
        static inline regType set1(int8_t a) { return vdupq_n_s8(a); }
        // 将32x4寄存器每个向量通道置为a1
        static inline col_regType set1_32(int32_t a) { return vdupq_n_s32(a); }
        // 返回两个8x16寄存器相应通道的较大值1
        static inline regType max(const regType &a, const regType &b) { return vmaxq_s8(a, b); }
        // 返回全0的8x16寄存器1
        static inline regType zero() { return vdupq_n_s8(0); }
        // 返回全0的32x4寄存器1
        static inline int32x4_t zero_32() { return vdupq_n_s32(0); }
        // 将向量寄存器的值存入内存中1
        static inline void store(regType *mem_addr, const regType &a) { vst1q_s8((int8_t *)mem_addr, a); }
        // 将向量寄存器的值存入内存中1
        static inline void store_32(col_regType *mem_addr, const col_regType &a) { vst1q_s32((int32_t *)mem_addr, a); }
        // 从内存中取出向量寄存器的值1
        static inline regType load(const regType *mem_addr) { return vld1q_s8((int8_t *)mem_addr); }
        // 从内存中取出向量寄存器的值1
        static inline col_regType load_32(const col_regType *mem_addr) { return vld1q_s32((int32_t *)mem_addr); }
        // 比较两个8x16寄存器各向量通道是否相等1
        static inline uint8x16_t cmpeq(const regType &a, const regType &b) { return vceqq_s8(a, b); }
        // 比较两个32x4寄存器个向量通道是否相等1
        static inline uint32x4_t cmpeq_32(const col_regType &a, const col_regType &b) { return vceqq_s32(a, b); }
        // 根据k的0/1返回a/b1
        static inline regType blend(uint8x16_t k, const regType &a, const regType &b) { return vbslq_s8(k, b, a); }
        // 根据k的1/0判断是否用b覆盖a
        static inline regType mask_set1(const regType &a, uint8x16_t uk, int8_t b)
        {
            // 向量c每个通道都置为b
            int8x16_t c = vdupq_n_s8(b);
            return vbslq_s8(uk, c, a);
        }
        // 根据k的1/0判断是否用b覆盖a
        static inline col_regType mask_set1_32(const col_regType &a, uint32x4_t uk, int32_t b)
        {
        ...
        // 返回(k3k2k1k0)
        static inline uint8x16_t combine_mask(uint32x4_t k0, uint32x4_t k1, uint32x4_t k2, uint32x4_t k3)
        {
            uint8x16_t k = vdupq_n_u8(0);
            if (vgetq_lane_u32(k0, 0) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 0);
            if (vgetq_lane_u32(k0, 1) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 1);
            if (vgetq_lane_u32(k0, 2) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 2);
            if (vgetq_lane_u32(k0, 3) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 3);
            if (vgetq_lane_u32(k1, 0) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 4);
            if (vgetq_lane_u32(k1, 1) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 5);
            if (vgetq_lane_u32(k1, 2) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 6);
            if (vgetq_lane_u32(k1, 3) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 7);
            if (vgetq_lane_u32(k2, 0) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 8);
            if (vgetq_lane_u32(k2, 1) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 9);
            if (vgetq_lane_u32(k2, 2) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 10);
            if (vgetq_lane_u32(k2, 3) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 11);
            if (vgetq_lane_u32(k3, 0) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 12);
            if (vgetq_lane_u32(k3, 1) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 13);
            if (vgetq_lane_u32(k3, 2) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 14);
            if (vgetq_lane_u32(k3, 3) == UINT32_MAX)
                k = vsetq_lane_u8(UINT8_MAX, k, 15);
            return k;
        }
        // 根据k的1/0判断是否用val覆盖c0123
        static inline void update_cols(col_regType &c0, col_regType &c1, col_regType &c2, col_regType &c3, int32_t val, uint8x16_t k)
        {
            if (vgetq_lane_u8(k, 0) == UINT8_MAX)
                c0 = vsetq_lane_s32(val, c0, 0);
            if (vgetq_lane_u8(k, 1) == UINT8_MAX)
                c0 = vsetq_lane_s32(val, c0, 1);
            if (vgetq_lane_u8(k, 2) == UINT8_MAX)
                c0 = vsetq_lane_s32(val, c0, 2);
            if (vgetq_lane_u8(k, 3) == UINT8_MAX)
                c0 = vsetq_lane_s32(val, c0, 3);
            if (vgetq_lane_u8(k, 4) == UINT8_MAX)
                c1 = vsetq_lane_s32(val, c1, 0);
            if (vgetq_lane_u8(k, 5) == UINT8_MAX)
                c1 = vsetq_lane_s32(val, c1, 1);
            if (vgetq_lane_u8(k, 6) == UINT8_MAX)
                c1 = vsetq_lane_s32(val, c1, 2);
            if (vgetq_lane_u8(k, 7) == UINT8_MAX)
                c1 = vsetq_lane_s32(val, c1, 3);
            if (vgetq_lane_u8(k, 8) == UINT8_MAX)
                c2 = vsetq_lane_s32(val, c2, 0);
            if (vgetq_lane_u8(k, 9) == UINT8_MAX)
                c2 = vsetq_lane_s32(val, c2, 1);
            if (vgetq_lane_u8(k, 10) == UINT8_MAX)
                c2 = vsetq_lane_s32(val, c2, 2);
            if (vgetq_lane_u8(k, 11) == UINT8_MAX)
                c2 = vsetq_lane_s32(val, c2, 3);
            if (vgetq_lane_u8(k, 12) == UINT8_MAX)
                c3 = vsetq_lane_s32(val, c3, 0);
            if (vgetq_lane_u8(k, 13) == UINT8_MAX)
                c3 = vsetq_lane_s32(val, c3, 1);
            if (vgetq_lane_u8(k, 14) == UINT8_MAX)
                c3 = vsetq_lane_s32(val, c3, 2);
            if (vgetq_lane_u8(k, 15) == UINT8_MAX)
                c3 = vsetq_lane_s32(val, c3, 3);
        }
#endif
    };
    ...
    public:
         ...
         */
        Phase1_Vectorized(const std::vector<std::string> &readSet, const CSR_char_container &g, const Parameters &p) : readSet(readSet), graph(g), parameters(p)
        {
        ...
        void alignToDAGLocal_Phase1_vectorized_wrapper(Vec &outputBestScoreVector) const
        {
            ...
            static_assert(colRegistersCountPerBatch == 1 || colRegistersCountPerBatch == 2 || colRegistersCountPerBatch == 4, "has to be either 1, 2 or 4");
            ...
            std::vector<__mxxxi, aligned_alloc<__mxxxi, 64>> _bestScoreColVector(countReadBatches * colRegistersCountPerBatch);
            std::vector<__mxxxi, aligned_alloc<__mxxxi, 64>> _bestScoreRowVector(countReadBatches);
#elif defined(PASGAL_ENABLE_NEON128)
            typedef typename SIMD::regType __mxxxi;
            std::vector<__mxxxi, aligned_alloc<__mxxxi, 64>> _bestScoreVector(countReadBatches);
            std::vector<int32x4_t, aligned_alloc<int32x4_t, 64>> _bestScoreColVector(countReadBatches * colRegistersCountPerBatch);
            std::vector<__mxxxi, aligned_alloc<__mxxxi, 64>> _bestScoreRowVector(countReadBatches);
#endif
            ...
            for (size_t i = 0; i < countReadBatches; i++)
            {
                ...
                {
#if defined(PASGAL_ENABLE_AVX512) || defined(PASGAL_ENABLE_AVX2)
                    SIMD::store((__mxxxi *)&storeCols[j * colValuesPerRegister], _bestScoreColVector[i * colRegistersCountPerBatch + j]);
#elif defined(PASGAL_ENABLE_NEON128)
                    SIMD::store_32((int32x4_t *)&storeCols[j * colValuesPerRegister], _bestScoreColVector[i * colRegistersCountPerBatch + j]);
                    // std::cout << vgetq_lane_s32(_bestScoreColVector[i * colRegistersCountPerBatch + j], 0) << std::endl;
                    // std::cout << vgetq_lane_s32(_bestScoreColVector[i * colRegistersCountPerBatch + j], 1) << std::endl;
                    // std::cout << vgetq_lane_s32(_bestScoreColVector[i * colRegistersCountPerBatch + j], 2) << std::endl;
                    // std::cout << vgetq_lane_s32(_bestScoreColVector[i * colRegistersCountPerBatch + j], 3) << std::endl;
#endif
                }
                // DEBUG
                // std::cout << "Scores after Phase1:\n";
                for (size_t j = 0; j < SIMD::numSeqs; j++)
                {
                    if (i * SIMD::numSeqs + j < readSet.size())
                    {
                        ...
                        // std::cout << originalReadId << ":" << (int)storeScores[j] << "\n";
                        // std::cout << originalReadId << ":" << outputBestScoreVector[originalReadId].score << " in [" << outputBestScoreVector[originalReadId].qryRowEnd << "," << outputBestScoreVector[originalReadId].refColumnEnd << "]\n";
                        // std::cout << originalReadId << ":" << storeScores[j] << " in [" << storeRows[j] << "," << storeCols[j] << "]\n";
 ...
 */
        ...
        void alignToDAGLocal_Phase1_vectorized(Vec1 &bestScores, Vec2 &bestCols, Vec1 &bestRows) const
#endif
        {
#ifdef PASGAL_ENABLE_NEON128
            typedef typename SIMD::regType __mxxxi;
#endif
...
#elif defined(PASGAL_ENABLE_NEON128)
            std::fill(bestCols.begin(), bestCols.end(), SIMD::zero_32());
#endif
...
#pragma omp for schedule(dynamic) nowait
...
#elif defined(PASGAL_ENABLE_NEON128)
                    int32x4_t bestCols512_0 = SIMD::zero_32();
                    int32x4_t bestCols512_1 = SIMD::zero_32();
                    int32x4_t bestCols512_2 = SIMD::zero_32();
                    int32x4_t bestCols512_3 = SIMD::zero_32();
#endif
    ...
    public:
         ...
         * @param[in]   g           input reference graph
         */
        Phase1_Rev_Vectorized(const std::vector<std::string> &readSet, const CSR_char_container &g, const Parameters &p) : readSet(readSet), graph(g), parameters(p)
        {
            this->sortReadsForLoadBalance();
            this->convertToSOA();
            this->computeLongHops();
        ...
        void alignToDAGLocal_Phase1_rev_vectorized_wrapper(Vec &outputBestScoreVector) const
        {
            ...
            for (size_t i = 0; i < countReadBatches; i++)
            {
                ...
                SIMD::store((__mxxxi *)storeRows.data(), _bestScoreRowVector[i]);
                // // DEBUG
                // std::cout << "Batch " << i << " Rev Scores:" << std::endl;
                // std::cout << "sizeof type:" << sizeof(typename SIMD::type) << std::endl;
                // for (int j = 0; j < SIMD::numSeqs; j++)
                // {
                //     std::cout << (int)storeScores[j] << " ";
                // }
                // std::cout << std::endl;
...
#ifdef DEBUG
                        std::cout << "INFO, psgl::Phase1_Vectorized::alignToDAGLocal_Phase1_rev_vectorized_wrapper, read # " << originalReadId
                                  << ",  score = " << (int)storeScores[j] << ", refColumnStart = " << (int)storeCols[j]
                                  << ", qryRowStart = " << (int)(readSet[originalReadId].length() - 1 - storeRows[j]) << "\n";
#endif
...
#if defined(PASGAL_ENABLE_AVX512) || defined(PASGAL_ENABLE_AVX2)
        template <typename Vec1, typename Vec2>
        void alignToDAGLocal_Phase1_rev_vectorized(const Vec1 &outputBestScoreVector, Vec2 &bestScores, Vec2 &bestCols, Vec2 &bestRows) const
#elif defined(PASGAL_ENABLE_NEON128)
        template <typename Vec1, typename Vec2, typename Vec3>
        void alignToDAGLocal_Phase1_rev_vectorized(const Vec1 &outputBestScoreVector, Vec2 &bestScores, Vec3 &bestCols, Vec2 &bestRows) const
#endif
        {
#ifdef PASGAL_ENABLE_NEON128
            typedef typename SIMD::regType __mxxxi;
...
#pragma omp for schedule(dynamic) nowait
...
#elif defined(PASGAL_ENABLE_NEON128)
                    int32x4_t fwdBestCols512_0;
                    int32x4_t fwdBestCols512_1;
                    int32x4_t fwdBestCols512_2;
                    int32x4_t fwdBestCols512_3;
#endif
...
#elif defined(PASGAL_ENABLE_NEON128)
                            fwdBestCols512_0 = SIMD::load_32((const int32x4_t *)&fwdBestCols[0]);

                            // storing best columns requires extra work
                            if (colRegistersCountPerBatch >= 2)
                                fwdBestCols512_1 = SIMD::load_32((const int32x4_t *)&fwdBestCols[1 * colValuesPerRegister]);

                            if (colRegistersCountPerBatch == 4)
                            {
                                fwdBestCols512_2 = SIMD::load_32((const int32x4_t *)&fwdBestCols[2 * colValuesPerRegister]);
                                fwdBestCols512_3 = SIMD::load_32((const int32x4_t *)&fwdBestCols[3 * colValuesPerRegister]);
                            }
#endif
                        }
                    }
...
#elif defined(PASGAL_ENABLE_NEON128)
                                    int32x4_t currentCol = SIMD::set1_32((int32_t)k);
#endif
            ...
            std::cout << "TIMER, psgl::alignToDAGLocal_Phase1_rev_vectorized"
                      << " (precision= " << sizeof(typename SIMD::type) << " bytes)"
                      << ", individual thread timings (s) : " << printStats(threadTimings) << "\n";
