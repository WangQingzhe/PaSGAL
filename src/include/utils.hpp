    ...
    namespace seqUtils
    {
        ...
        void cigarCompact(std::string &cigar)
        {
            ...
            for (auto it = cigarCpy.begin(); it != cigarCpy.end();)
            {
                ...
                auto it2 = std::find_if(it, cigarCpy.end(), [&](const char &c)
                                        { return c != current; });
...
#pragma omp parallel
    ...
    public:
        ...
        template <typename U>
        aligned_alloc(const aligned_alloc<U, alignment> &) {}
...
#elif defined(PASGAL_ENABLE_NEON128)
            void *const pv = neon_mm_malloc(n * sizeof(Type), alignment);
#else
            void *const pv = NULL;
#endif
    ...
    private:
        ...
        static void *neon_mm_malloc(size_t __size, size_t __alignment)
        {
            void *__ptr;
            if (__alignment == 1)
